...
export async function POST(
  ...
  try {
    ...
    const submission = getSubmission(post.submissionId);
    if (!submission) return json({ error: "Submission not found" }, { status: 404 });
    if (submission.mediaType !== "image") {
      return json({ error: "Caption regeneration only supports images" }, { status: 409 });
    }

    const campaign = getCampaign(post.campaignId);
    ...
    const socialPost = updateDraftSocialPostCaption(
      id,
      captionWithHashtags(result.caption, result.hashtags),
    );
